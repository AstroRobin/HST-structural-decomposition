...
## Usage:
# >> Rscript Model_HST_Galaxies.R --inputcat=[PATH] --model=[MODEL] --computer=['local'|'magnus']
...
#args = list(inputcat = '/Users/00092380/Documents/GoogleDrive/PostDoc-UWA/Tasks/DEVILS_Structural_Decomposition/Catalogues/Subcats/Pilot/DEVILS_HST_pilot_subcat_0.csv', model = 'single', computer = 'local', name = 'test')
...
for (idx in seq(1,nrow(dfCat))){
  ...
  expNameList = c() # The names of each exposure
  ...
  for ( jj in seq(1,numExps) ){
    ...
    expNameList = c(expNameList, expName)
    print(expName)
    ...
    imgList[[expName]] = hdulist[[sciIndex]]$imDat
    dqList[[expName]] = profoundMakeSegimDilate(segim=hdulist[[dqIndex]]$imDat,size=3,expand=c(4096,8192))$segim # Expand the cosmic rays by a pixel either side (i.e. size=3)
    hdrList[[expName]] = hdulist[[sciIndex]]$hdr
  
    # Now get cutouts
    ### TODO: change this to Rwcs_cutout
    print(glue("INFO: Source location in {expName}: {srcLoc[1]}, {srcLoc[2]}"))
    imgCutList[[expName]] = magcutout(imgList[[expName]], loc=srcLoc, box=cutoutBox, plot=FALSE)$image
    
    dqCutList[[expName]] = magcutout(dqList[[expName]], loc=srcLoc, box=cutoutBox, plot=FALSE)$image
    maskList[[expName]] = apply(apply(dqCutList[[expName]], c(1,2), mask_pixel), c(1,2), as.integer)
    ...
    skyList[[expName]] = skyGrid$sky
    skyRMSList[[expName]] = skyGrid$skyRMS
    ...
    if (!file.exists(psfFilename) | clobberPSF==TRUE){
      ...
      #system(glue("Rscript {psfScriptPath} {psfsDir} {sourceName}_{dfCat$name_exp1[idx]} {dfCat$chip_exp1[idx]} {dfCat$x_exp1[idx]} {dfCat$y_exp1[idx]} 0.0 {sizePSF}"))
    }
  ...
  print("INFO: Names of imgCutList:")
  print(names(imgCutList))
  
  # Stack the images and masks
  print(paste0("INFO: Creating median stack of ", length(imgCutList)," cutout images."))
  imgCutStack = profoundMakeStack(image_list=imgCutList, sky_list=skyList, skyRMS_list=skyRMSList, mask_list=maskList, magzero_in = magZP)$image   #imgCutStack = median_stack(images=imgCutList, dqmaps=dqCutList, plot=FALSE)
  ...
  if (dfCat$num_exps[idx] > 1){
    for (ii in seq(2,min(c(dfCat$num_exps[idx], maxExps)))){
  ...
  seg = profoundProFound(image=imgCutStack, mask=maskStack, sigma=1.5, dilete=5, skycut=1.0, SBdilate=2.5, tol=5, reltol=0.25, ext=5, plot=toPlot)
  ...
  for (jj in seq(1,numExps)){
    ...
    skyList[[expName]] = profoundProFound(image=imgCutList[[jj]], segim=seg$segim, mask=maskList[[jj]],magzero = magZP, gain=ccdGain,
                                          sigma=1.5, skycut=0.75, SBdilate=3, size=7, box=c(150,150))#skyBoxDims)
  ...
  if (toPlot|toSave){
    ...
    for (jj in seq(1,numExps)){
      profoundSegimPlot(image=imgCutList[[jj]], segim=seg$segim, mask=maskList[[jj]], bad=NA)
  ...
  segimList = replicate(numExps, seg$segim, simplify = F)
  ...
  for (jj in seq(1,numExps)){ 
    offsets[[jj]] = c(dfCat[[glue('x_exp{jj}')]][idx]%%1 - dfCat[['x_exp1']][idx]%%1, dfCat[[glue('y_exp{jj}')]][idx]%%1 - dfCat[['y_exp1']][idx]%%1)
  }
