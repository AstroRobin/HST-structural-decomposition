...
parser$add_argument("-m","--models",
                    action='store',dest='models',type='character',nargs='+',default=NULL,choices=c('single','psfexp','devexp','psfser','devser','serexp','serser'),
...
get_model_opts = function(model){
  ...
  } else if (model == 'psfexp'){ # PSF bulge + Exponential disk
    modelOpts = c(modelOpts, 
                  list(n_comps = 1.5,
                       sing_nser_fit = FALSE,
                       disk_nser_fit = FALSE,
                       bulge_nser_fit = FALSE))
  } else if (model == 'devexp'){  # de Vaucouleurs bulge + Exponential disk (Canonical light profiles)
    modelOpts = c(modelOpts, 
                  list(n_comps = 2,
                       sing_nser_fit = FALSE,
                       disk_nser_fit = FALSE,
                       bulge_nser_fit = FALSE))
  } else if (model == 'psfser'){  # PSF bulge + Free disk
    modelOpts = c(modelOpts, 
                  list(n_comps = 1.5,
                       sing_nser_fit = FALSE,
                       disk_nser_fit = TRUE,
                       bulge_nser_fit = FALSE))
  } else if (model == 'devser'){  # de Vaucouleurs bulge + Free disk
    modelOpts = c(modelOpts, 
                  list(n_comps = 2,
                       sing_nser_fit = FALSE,
                       disk_nser_fit = TRUE,
                       bulge_nser_fit = FALSE))
  } else if (model == 'serexp'){  # Free bulge + Exponential disk (pseudobulges?)
    modelOpts = c(modelOpts, 
                  list(n_comps = 2,
                       sing_nser_fit = FALSE,
                       disk_nser_fit = FALSE,
                       bulge_nser_fit = TRUE))
  } else if (model == 'serser'){  # Free bulge + Free disk (Most flexibile model, least constrained)
    modelOpts = c(modelOpts, 
                  list(n_comps = 2,
                       sing_nser_fit = FALSE,
                       disk_nser_fit = TRUE,
...
foreach(idx=1:nrow(dfCat), .inorder=FALSE) %dopar% {
  check=try({ # Catch any failed runs
    ...
    if (logsToFile == TRUE){
      ...
      logFile = paste0(logsDir,'/D',sourceName,'_log.txt')
      sink(logFile)
    } else {
      logFile = ''
    }
    ...
    if (numExps == 0){
      ...
      if (toPlot|toSave){
        if (toSave){png(filename = paste0(plotDir,'/',sourceName,'/D',sourceName,'_segmentation_map.png'), width=650, height=650, pointsize=20)}
      ...
      if (toPlot|toSave){
        if (numExps <= 4){
          if (toSave){png(filename = paste0(plotDir,'/',sourceName,'/D',sourceName,'_cutouts.png'), width=900, height=900, pointsize=10)}
          par(mfrow=c(2,2), mar = c(1.5,1.5,1.5,1.5), oma = c(0.25,0.25,0.25,0.25))
        } else if (numExps > 4 & numExps <= 8) {
          if (toSave){png(filename = paste0(plotDir,'/',sourceName,'/D',sourceName,'_cutouts.png'), width=1800, height=900, pointsize=10)}
          par(mfrow=c(2,4), mar = c(1.5,1.5,1.5,1.5), oma = c(0.25,0.25,0.25,0.25))
        } else if (numExps > 8 & numExps <= 12){
          if (toSave){png(filename = paste0(plotDir,'/',sourceName,'/D',sourceName,'_cutouts.png'), width=1800, height=900+450, pointsize=10)}
          par(mfrow=c(3,4), mar = c(1.5,1.5,1.5,1.5), oma = c(0.25,0.25,0.25,0.25))
        } else {
          if (toSave){png(filename = paste0(plotDir,'/',sourceName,'/D',sourceName,'_cutouts.png'), width=1800, height=1800, pointsize=10)}
      ...
      # if (toSave){CairoPNG(filename = paste0(plotDir,'/',sourceName,'/D',sourceName,'_offsets.png'), width=3000, height=1500*(1+(numExps-1)%/%4), pointsize=20)}
      if (toPlot){
      ...
      outputFilename = paste0(resultsDir,'/',sourceName,'/D',sourceName,'_output.rds')
